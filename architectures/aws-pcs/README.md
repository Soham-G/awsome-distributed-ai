...
| [`pcs-rocky9-cluster-all-in-one.yaml`](./assets/pcs-rocky9-cluster-all-in-one.yaml) | **One stack: build the Rocky 9 GPU AMI *and* deploy the whole cluster on it** (nests the builder + `pcs-ml-cluster-deploy-all.yaml`). Pass `AmiId` to skip the build and reuse a prior AMI. This is the default path in [G7E-DEPLOY.md](./docs/G7E-DEPLOY.md) | [![Launch](images/launch-stack.svg)](https://console.aws.amazon.com/cloudformation/home#/stacks/quickcreate?templateUrl=https://awsome-distributed-ai.s3.amazonaws.com/templates/pcs-rocky9-cluster-all-in-one.yaml&stackName=pcs-rocky9-cluster) |
