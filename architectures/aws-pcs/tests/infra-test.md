...
>   deploy a cluster pinned to it (`AmiId=<ami-xxx>` + `PostInstallScriptUrl=none`), and
>   run a container job.
...
`PostInstallScriptUrl=none` so nothing else runs at boot.
...
test ! -s /var/log/pcs-post-install.log && echo "post-install did not run (PostInstallScriptUrl=none)"
```
...
post-install hook running (because `PostInstallScriptUrl=none`); the container job
